...
@section('content')
    <main class="pt-90">
        ...
        <div class="row">
            ...
            <div class="product-single__media" data-media-type="vertical-thumbnail">
                ...
                <div class="swiper-container">
                        ...
                        @if (!empty($product))
                            @foreach (json_decode($product->images, true) ?? [] as $image)
                                <div class="swiper-slide product-single__image-item">
                                    ...
                                    data-bs-placement="left" title="Zoom" target="_blank">
                    ...
                    <div class="swiper-button-prev"><svg width="7" height="11" viewBox="0 0 7 11"
                        xmlns="http://wwwN/A.w3.org/2000/svg">
                        ...
                        @if (!empty($product))
                            @foreach (json_decode($product->images, true) ?? [] as $image)
                                <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
                                    src="{{ asset('uploads/products/' . $image) }}" width="104" height="104" alt="{{ $product->name }}" /></div>
                            @endforeach
                        @endif
                    </div>
                </div>
            ...
            <h1 class="product-single__name">{{ $product->brand->name . ' ' . $product->name }}</h1>
            ...
            <div class="product-single__price">
                <span class="current-price">
                    @if (!empty($product->sale_price) && $product->sale_price != 0)
                        <del>R{{ $product->regular_price }}</del> R{{ $product->sale_price }}
                    @else
                        ...
                        <div class="swiper-slide product-card">
                            <div class="pc__img-wrapper">
                                @if (!empty($relatedProduct->images))
                                    @php $count = 1; @endphp
                                    @foreach (json_decode($relatedProduct->images, true) ?? [] as $index => $image)
                                        <a href="{{ route('shop.product-details', $relatedProduct->slug) }}">
                                            @if ($count == 1 && isset(json_decode($relatedProduct->images, true)[$index]))
                                                <img loading="lazy" src="{{ asset('uploads/products/' . $image) }}" width="330" height="400"
                                                    alt="{{ $relatedProduct->name }}" class="pc__img">
                                            @endif
                                            @if ($count == 2 && isset(json_decode($relatedProduct->images, true)[$index + 1]))
                                                <img loading="lazy" src="{{ asset('uploads/products/' . json_decode($relatedProduct->images, true)[$index + 1]) }}" 
                                                    width="330" height="400" alt="{{ $relatedProduct->name }}" class="pc__img pc__img-second">
                                            @endif
                                        </a>  
                                        @php $count++; @endphp
                                    @endforeach
                            ...
                            <div class="pc__info position-relative">
                                ...
                                <h6 class="pc__title"><a href="details.html">{{ $relatedProduct->name }}</a></h6>
                                <div class="product-card__price d-flex">
                                    <span class="money price">
                                        @if (!empty($relatedProduct->sale_price) && $relatedProduct->sale_price != 0)
                                            <del>R{{ $relatedProduct->regular_price }}</del> R{{ $relatedProduct->sale_price }}
                                        @else
                                            R{{ $prorelatedProductduct->regular_price }}
                                        @endif
                                    </span>
                                </div>
